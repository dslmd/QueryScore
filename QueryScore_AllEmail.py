...
# Open and get the score on the transcript, it should contain four parts in sequence: name, student ID, score, email
f = open('scores.txt')
s = f.read()
...
while (1):
    ...
    if (pop3_server.list()[1]!=[]):
        ...
        for i in range(student_number):
            if (searchname == name[i]) and (searchnumber == number[i]) and (addr == corr_addr[i]):
                searchgrade = grade[i]
